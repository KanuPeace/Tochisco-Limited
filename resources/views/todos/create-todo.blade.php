...
<div class="max-w-2xl px-3 py-3 mx-auto mt-10 md:px-0 md:justify-between md:items-center">
    ...
    <div class="flex flex-col items-center justify-center mt-2">
        <h3 class="mb-5 text-lg font-bold text-purple-800">Create Todo</h3>
        <form action="{{ route('todo.store') }}" method="post">
            ...
            <div class="mt-7">
                ...
                <textarea name="content" id="content"
                    ...
                    cols="30" rows="3">{{ old('content') }}</textarea>
                @error('content')
                    <div class="mt-3 text-xs text-red-700">{{ $message }}</div>
                @enderror
            </div>
            <div class="mt-7">
                <button type="submit"
                    class="w-full px-5 py-2 text-lg font-semibold text-white bg-purple-600 rounded-md hover:bg-purple-700 md:w-96 items-right focus:outline-none focus:border-none focus:ring-1">Send</button>
            </div>
        </form>
    </div>
</div>
